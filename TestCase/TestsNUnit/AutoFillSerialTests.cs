...
namespace unvell.ReoGrid.Tests.TestsNUnit
{
	...
	public class AutoFillSerialTests
	{
		...
		[Test]
		public void PR567_AutoFillSerial_IgnoresHiddenRows_FilteredByText()
		{
			// Arrange
			sheet["A1"] = "Test1";
			sheet["A2"] = "Test1";
			...
			// Create auto filter on column A only (no GUI)
			var filter = sheet.CreateColumnFilter("A", "A", 0, AutoColumnFilterUI.NoGUI);
			var colAFilter = filter.Columns["A"];
			colAFilter.IsSelectAll = false;
			colAFilter.SelectedTextItems.Clear();
			colAFilter.SelectedTextItems.Add("Test1");
			filter.Apply();

			// Verify visibility after filter
			Assert.IsTrue(sheet.IsRowVisible(0)); // row 1
			Assert.IsTrue(sheet.IsRowVisible(1)); // row 2
			Assert.IsFalse(sheet.IsRowVisible(2)); // row 3 hidden ("Test2")
			Assert.IsTrue(sheet.IsRowVisible(3)); // row 4
			Assert.IsFalse(sheet.IsRowVisible(4)); // row 5 hidden (blank)
			Assert.IsTrue(sheet.IsRowVisible(5)); // row 6

			// Act: drag-and-drop from A1 to A6 (programmatically)
			sheet.AutoFillSerial("A1", "A2:A6");

			// Assert: hidden cells should be ignored and unchanged
			Assert.AreEqual("Test2", sheet["A3"], "Hidden row A3 should not be changed by AutoFillSerial.");
			Assert.AreEqual(null, sheet["A5"], "Hidden row A5 should remain unchanged (null). ");

			// Visible cells should be Test1
			Assert.AreEqual("Test1", sheet["A2"]);
			Assert.AreEqual("Test1", sheet["A4"]);
			Assert.AreEqual("Test1", sheet["A6"]);
		}
	}
}
